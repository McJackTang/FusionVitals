...
class PhysnetTrainer(BaseTrainer):
    ...
    def __init__(self, config, data_loader):
        ...
        self.dataset_type = config.DATASET_TYPE # 接收数据集类型 face finger not task
            ...
            for idx, batch in enumerate(tbar):
                ...
                if self.dataset_type != "both":
                    if self.task == "bvp":
                        ...
                        # print(f"batch_shape: {len(batch)}")
                        # print(type(batch[3]),batch[3], type(batch[3]))
        ...
        with open('/data2/lk/rppg-toolbox/result.csv', 'a', newline='') as csvfile:
            ...
            if self.task == "bvp":
                ...
                data_to_add = [
                    ...
                    "/", "/", "/", "/", "/", "/",
                    "/", "/", "/","/",
                    self.model_name, self.train_state, self.test_state
                ]
            elif self.task == "spo2":
