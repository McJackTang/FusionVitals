...
class BaseLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.dataset_type == "face_IR":
            ...
            data = np.transpose(data, (3, 0, 1, 2))
            data = np.float32(data)
            item_path = self.inputs_face_IR[index]
            item_path_filename = item_path.split(os.sep)[-1]
            split_idx = item_path_filename.rindex('_input_')
            filename = item_path_filename[:split_idx]
            chunk_id = item_path_filename[split_idx + len('_input_'):].split('.')[0]
        ...
        if self.dataset_type == "both":
            ...
            data_face_IR = np.load(self.inputs_face_IR[index])
            data_face = np.transpose(data_face, (3, 0, 1, 2))
            data_face_IR = np.transpose(data_face_IR, (3, 0, 1, 2))
            data_face = np.float32(data_face)
            data_face_IR = np.float32(data_face_IR)
            item_path = self.inputs[index]
            item_path_filename = item_path.split(os.sep)[-1]
            split_idx = item_path_filename.rindex('_input_')
            filename = item_path_filename[:split_idx]
    ...
    def generate_pos_psuedo_labels(self, frames, fs=30, min_freq = 0.70, max_freq = 3):
    ...
    def save_multi_process(self, frames_clips, bvps_clips, spo2_clips, rr_clips, filename):
        ...
        Args:
            frames_clips(np.array): video frames data
    ...
    def load_preprocessed_data(self):
        ...
        labels_rr = [input_file.replace("face_input", "rr") for input_file in inputs_only_face]
        ...
        print(f"Loaded {len(self.inputs)} inputs, {len(self.labels_bvp)} BVP labels, "
              f"{len(self.labels_spo2)} SpO2 labels, {len(labels_rr)} RR labels")
