...
class BaseLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if label_spo2.ndim == 1:
            label_spo2 = label_spo2.reshape(128, 1)

        # 将 label_spo2 从 [128, 1] 转换为 [4, 1]
        label_spo2 = label_spo2.reshape(4, -1).mean(axis=1, keepdims=True)

        # print("label_spo2", label_spo2)

        # item_path is the location of a specific clip in a preprocessing output folder
        item_path = self.inputs[index]
        # item_path_filename is simply the filename of the specific clip
        item_path_filename = item_path.split(os.sep)[-1]
        # Find the last underscore before the numeric chunk identifier
        split_idx = item_path_filename.rindex('_input_')
        # Extract the filename prefix and chunk identifier
        filename = item_path_filename[:split_idx]
    ...
    def preprocess_raw(self, frames, bvps, config_preprocess):
    ...
    def preprocess(self, frames, bvps, spo2, config_preprocess):
        ...
        Args:
            frames(np.array): Frames in a video.
            bvps(np.array): Blood volumne pulse (PPG) signal labels for a video.
            spo2(np.array): SpO2 signal labels for a video.
            config_preprocess(CfgNode): preprocessing settings(ref:config.py).

        Returns:
            frame_clips(np.array): processed video data by frames
            bvps_clips(np.array): processed bvp (ppg) labels by frames
            spo2_clips(np.array): processed SpO2 labels by frames
        """
        # Resize frames and crop for face region
        frames = self.crop_face_resize(
            frames,
        ...
        if config_preprocess.LABEL_TYPE == "Raw":
            ...
            bvps = BaseLoader.diff_normalize_label(bvps)
            # spo2 = BaseLoader.diff_normalize_label(spo2)
        elif config_preprocess.LABEL_TYPE == "Standardized":
            bvps = BaseLoader.standardized_label(bvps)
            # spo2 = BaseLoader.standardized_label(spo2)
        else:
            raise ValueError("Unsupported label type!")

        if config_preprocess.DO_CHUNK:  # chunk data into snippets
            frames_clips, bvps_clips = self.chunk(data, bvps, config_preprocess.CHUNK_LENGTH)
            _, spo2_clips = self.chunk(data, spo2, config_preprocess.CHUNK_LENGTH)
        else:
            frames_clips = np.array([data])
            bvps_clips = np.array([bvps])
    ...
    def save_multi_process2222(self, frames_clips, bvps_clips, filename):
    ...
    def save_multi_process(self, frames_clips, bvps_clips, spo2_clips, filename):
        """Save all the chunked data with multi-thread processing.
        ...
        for i in range(len(bvps_clips)):
            ...
            label_path_name = os.path.join(self.cached_path, f"{filename}_label_{count}.npy")
    ...
    def load_preprocessed_data(self):
        ...
        labels_bvp = [input_file.replace("face_input", "face_hr") for input_file in inputs]
        labels_spo2 = [input_file.replace("face_input", "face_spo2") for input_file in inputs]
        self.inputs = inputs
