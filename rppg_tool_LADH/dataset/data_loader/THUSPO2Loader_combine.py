...
class THUSPO2Loader(BaseLoader):
    ...
    def preprocess_dataset_subprocess(self, data_dirs, config_preprocess, i, file_list_dict):
        ...
        video_file_face = data_dirs[i]['path']
        video_file_finger = video_file_face.replace('face', 'finger')
        frames_face = self.read_video(video_file_face)
        frames_finger = self.read_video(video_file_finger)

        # Get the directory of the current video
        video_dir = os.path.dirname(video_file_face)
        ...
        if config_preprocess.USE_PSUEDO_PPG_LABEL:
            bvps = self.generate_pos_psuedo_labels(frames_face, fs=self.config_data.FS)
        else:
            bvps = resampled_bvp
            spo2 = resampled_spo2

        frames_clips_face, bvps_clips, spo2_clips = self.preprocess(frames_face, bvps, spo2, config_preprocess)
        frames_clips_finger, _, _ = self.preprocess(frames_finger, bvps, spo2, config_preprocess)

        # Create a unique filename that includes the subject ID and experiment ID
        filename = f"{subject_id}_{experiment_id}_{data_dirs[i]['index']}"

        # Save processed data and update the file list dictionary with filenames including subject ID and experiment ID
        input_name_list_face, label_name_list, spo2_name_list = self.save_multi_process(frames_clips_face, bvps_clips, spo2_clips, filename + "_face")
        input_name_list_finger, _, _ = self.save_multi_process(frames_clips_finger, bvps_clips, spo2_clips, filename + "_finger")

        file_list_dict[i] = input_name_list_face + input_name_list_finger
    ...
    def get_dataset_files(self, data_path):
        ...
        video_file_face = glob.glob(os.path.join(data_path, "video_ZIP_H264_face.avi"))[0]
        video_file_finger = glob.glob(os.path.join(data_path, "video_ZIP_H264_finger.avi"))[0]
        ...
        return video_file_face, video_file_finger, bvp_file, timestamp_file, spo2_file
    ...
    def test_bvp_resampling(self, dataset_path):
        ...
        video_file_face, video_file_finger, bvp_file, timestamp_file, spo2_file = self.get_dataset_files(dataset_path)
        ...
        print("Number of frames (face): ", len(frames_face))
        print("Number of frames (finger): ", len(frames_finger))
